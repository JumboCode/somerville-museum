...
export async function searchHandler(req, res) {
    ...
    FROM dummy_data 
    LEFT JOIN borrowers 
    ON dummy_data.current_borrower = borrowers.id 
    WHERE dummy_data.name ILIKE '%' || $1 || '%' 
       OR dummy_data.notes ILIKE '%' || $1 || '%' 
       OR CAST(dummy_data.id AS TEXT) ILIKE '%' || $1 || '%' 
       OR borrowers.name ILIKE '%' || $1 || '%' 
